...
function DropdownMenuItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn(
        // 对齐原版 xboard（旧版 shadcn + 原版自加 cursor-pointer）；保留 destructive 变体能力
        'relative flex cursor-default cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[inset]:pl-8 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive',
        className
      )}
      {...props}
    />
...
function DropdownMenuLabel({
  ...
  return (
    <DropdownMenuPrimitive.Label
      ...
      data-inset={inset}
      className={cn(
        // 对齐原版 xboard（旧版 shadcn）：font-semibold
...
function DropdownMenuShortcut({
  ...
  return (
    <span
      ...
      className={cn(
        // 对齐原版 xboard（旧版 shadcn）：opacity-60
